...
public class KfakDdlTOHiveTable {
...
//        String insertSql = "insert into  kkk SELECT aa,bb,DATE_FORMAT(bb, 'yyyy-MM-dd'), DATE_FORMAT(bb, 'HH'), DATE_FORMAT(bb, 'mm') FROM ooo";
     //   bsTableEnv.executeSql(insertSql);


//        String queryDdl="SELECT aa,bb FROM ooo";
...
//
        String insertSql = "insert into  order115 SELECT id,commodity_id,order_num,usercode,status,amount,phone_num,create_time," +
                "addr,order_flag,DATE_FORMAT(create_time, 'yyyy-MM-dd'), DATE_FORMAT(create_time, 'HH'), DATE_FORMAT(create_time, 'mm')" +
                " FROM user_behavior002";
        bsTableEnv.executeSql(insertSql);




//        String queryDdl="SELECT id,commodity_id,order_num,usercode,status,amount,phone_num,create_time,addr,order_flag FROM order114";
//        Table table = bsTableEnv.sqlQuery(queryDdl);
//        DataStream<Tuple2<Boolean, Row>> tuple2DataStream = bsTableEnv.toRetractStream(table, Row.class);
//        tuple2DataStream.print().setParallelism(1);
//        env.execute("090");
